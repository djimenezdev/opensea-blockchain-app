...
const GeneralDetails = ({selectedNft}:{selectedNft:any}) => {
  ...
  useEffect(() => {
    (async () => {
      const user = await fetch(`https://rinkeby-api.opensea.io/user/${selectedNft?.owner}`).then((res) => res.json());
      if(user?.username) {
  ...
  return (
    <div className={style.wrapper}>
        <div className={style.infoContainer}>
            <div className={style.accent}>DJ PUDGY Pens</div>
        ...
        <div className={style.actionButtonsContainer}>
            <div className={style.actionButtons}>
                <div className={`${style.actionButton} ml-2`}><MdRefresh /></div>
