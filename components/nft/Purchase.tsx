...
const Purchase = ({
  ...
  const marketplace = useMarketplace(
    '0xF1402ECdDeC1a83de2a49dd0e0eF25925Ad47892'
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      setSelectedMarketNft(
        listings?.find(
          (marketNft) =>
            parseInt(marketNft?.asset?.id?._hex, 16) ===
            parseInt(selectedNft?.metadata?.id?._hex, 16)
        )
      );
    })();
  ...
  const confirmPurchase = async (toastHandler = toast) => {
    ...
    await fetch('https://os-market-clone.vercel.app/api/collectionData', {
      method: 'POST',
  ...
  const buyItem = async (listingId: number, quantityDesired: number) => {
    const res = await marketplace?.direct
