...
const EventItem = ({event}:{event:any}) => {
  return (
   <div className={style.eventItem}>
       ...
       <div className={`${style.accent} flex-[3]`}>{event.from.substring(0,2) + "..." + event.from.substring(event.from.length-2, event.from.length)}</div>
       <div className={`${style.accent} flex-[3]`}>{event.to.substring(0,2) + "..." + event.to.substring(event.to.length-3, event.to.length+1)}</div>
       <div className={`${style.accent} flex-[2] cursor-pointer underline`}><a target="_blank" rel="noreferrer" href={`https://rinkeby.etherscan.io/tx/${event.transaction_hash}`}>{event.date}</a></div>
   </div>
  )
}
